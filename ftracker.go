...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	midSpeedSec := meanSpeed(action, duration) * kmhInMsec
	heightM := height / cmInM
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(midSpeedSec, 2)/heightM)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
